...
Self-contained builder for the KOSMOS model that:
- Uses KOSMOS_Plat.e2k as input (no manual config edits)
- Writes Phase-1 artifacts to ./out_Kosmos/
...
# Phase-1 & config (we’ll override these in-memory for Kosmos only)
...
KOSMOS_E2K_FILE = Path("KOSMOS_Plat.e2k")
OUT_DIR         = Path("out_Kosmos")
# --------------------------------------------
...
def _run_phase1_for_kosmos(verbose: bool = True) -> None:
    """
    Run Phase-1 for KOSMOS into OUT_DIR, without permanently touching your global config.
    We temporarily patch config.E2K_PATH and config.OUT_DIR, reload phase1_run, and run it.
    """
    if verbose:
        print(f"[KOSMOS] Phase-1: E2K_FILE = {KOSMOS_E2K_FILE.resolve()}")
        print(f"[KOSMOS] Phase-1: OUT_DIR  = {OUT_DIR.resolve()}")

    if not KOSMOS_E2K_FILE.exists():
        raise FileNotFoundError(f"[KOSMOS] Input .e2k not found at: {KOSMOS_E2K_FILE.resolve()}")
    ...
    try:
        ...
        if verbose:
            ...
            print("[KOSMOS] Phase-1 complete.")
            print(f"  - {rp}")
            print(f"  - {sp}")

    finally:
        # Restore original config (don’t affect your other translators)
...
def _preflight_phase1():
    ...
    if _needs_phase1(KOSMOS_E2K_FILE, OUT_DIR):
        _run_phase1_for_kosmos(verbose=True)
...
def build_model(stage: str = "all") -> None:
    """
    Build the KOSMOS model in stages, using KOSMOS-specific artifacts in out_Kosmos/.
